...
const STEP_COMPONENTS: Record<UserRole, React.ComponentType<any>[]> = {
  [UserRole.SUPPORT_WORKER]: [WorkerStep01_Identity,WorkerStep02_RightToWork,WorkerStep03_WorkType,WorkerStep04_Services,WorkerStep05_Availability,WorkerStep06_ServiceAreas,WorkerStep07_Financials,WorkerStep08_Documents,WorkerStep09_Compliance],
  [UserRole.PROVIDER]:       [ProviderStep01_Business,ProviderStep02_ABN,ProviderStep03_PrimaryContact,ProviderStep04_AccountsContact,ProviderStep05_Logo,ProviderStep06_Services,ProviderStep07_ServiceAreas,ProviderStep08_Workforce,ProviderStep09_Pricing,ProviderStep10_About,ProviderStep11_Documents,ProviderStep12_Declaration],
  [UserRole.COORDINATOR]:    [CoordStep01_RoleOrg,CoordStep02_NDIS,CoordStep03_Experience,CoordStep04_Coverage,CoordStep05_Capacity,CoordStep06_Billing,CoordStep07_Bio,CoordStep08_Documents,CoordStep09_Declaration],
  [UserRole.PARTICIPANT]:    [ParticipantStep01_Personal,ParticipantStep02_NDIS,ParticipantStep03_SupportNeeds,ParticipantStep04_EmergencyContact,ParticipantStep05_Declaration],
...
export default function RegisterPage() {
  ...
  async function handleOtpResend() {
    ...
    try { await api.post('/auth/verify/resend', { channel:'phone' }); setOtpResent(true); }
    ...
    <AuthLayout mode="register">
      ...
      {phase === 'payment' && (
        <>
          ...
          <div style={{background:'#E8F5E9',border:'1px solid #A5D6A7',borderRadius:10,padding:'10px 14px',fontSize:12,color:'#2E7D32',marginBottom:20,display:'flex',alignItems:'center',gap:8}}>
            <i className="bi bi-shield-check-fill" />
            Test mode &mdash; any card number is accepted
          </div>

          <form onSubmit={handlePayment} style={{display:'flex',flexDirection:'column',gap:14}}>
            <div>
              <label style={lbl}>Card Number</label>
              <input type="text" value={cardNumber} onChange={e=>setCardNumber(fmtCard(e.target.value))} placeholder="1234 5678 9012 3456" style={inp} maxLength={19} inputMode="numeric" />
            </div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
              <div>
                <label style={lbl}>Expiry</label>
                <input type="text" value={expiry} onChange={e=>setExpiry(fmtExpiry(e.target.value))} placeholder="MM/YY" style={inp} maxLength={5} inputMode="numeric" />
              </div>
              <div>
                <label style={lbl}>CVV</label>
                <input type="text" value={cvv} onChange={e=>setCvv(e.target.value.replace(/\D/g,'').slice(0,4))} placeholder="123" style={inp} maxLength={4} inputMode="numeric" />
              </div>
            </div>
            <div>
              <label style={lbl}>Name on Card</label>
              <input type="text" value={cardName} onChange={e=>setCardName(e.target.value)} placeholder="Jane Smith" style={inp} autoComplete="cc-name" />
            </div>

            {payError && (
              <div style={{background:'#FFF0F0',border:'1px solid #FFCDD2',borderRadius:10,padding:'10px 14px',fontSize:13,color:'#C62828'}}>{payError}</div>
            )}

            <button type="submit" disabled={payLoading} className="btn-shiftify"
              ...
              {payLoading ? 'Processing...' : `Pay $${selectedPlanData ? planPrice(selectedPlanData).toFixed(2) : '--'} & Continue`}
            </button>
          </form>
        </>
      )}
