'use client';
// /setup/profile — Profile setup wizard, runs after phone OTP + plan payment.
// Reads activeRole from auth store to select the right step config.
// Guards: must be authenticated (middleware handles) + must have completed setup funnel.
  ...
  if (loading || !user) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--clr-bg)' }}>
        <div style={{ width: 36, height: 36, border: '3px solid var(--clr-border)', borderTopColor: 'var(--clr-primary)', borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} />
      </div>
    );
  }

  if (!activeRole || !STEP_COMPONENTS[activeRole]) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 12, padding: 24 }}>
        <i className="bi bi-exclamation-circle-fill" style={{ fontSize: 40, color: '#f59e0b' }} />
        <p style={{ fontSize: 14, color: 'var(--clr-text)' }}>No active role found. Please <a href="/login" style={{ color: 'var(--clr-primary)' }}>log in</a> again.</p>
      </div>
    );
  }

  const steps     = getStepsForRole(activeRole);
  const stepComps = STEP_COMPONENTS[activeRole];

  return (
    <WizardShell
      role={activeRole}
      steps={steps}
      stepComponents={stepComps}
    />
  );
}
