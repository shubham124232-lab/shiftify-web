// components/MarketplaceSection.jsx
const listings = [
  {
    type: 'emergency',
    title: 'Emergency — Personal Care',
    location: 'Melbourne, VIC',
    time: 'Needed ASAP',
    rate: '$45/hr',
    tags: ['Urgent', 'NDIS Funded'],
    initials: 'TW',
    color: '#DC2626',
    name: 'Thomas W.',
  },
  ...
  {
    type: 'regular',
    title: 'Overnight Support Worker',
    location: 'Brisbane, QLD',
    time: 'Fri & Sat nights',
    rate: '$52/hr',
    tags: ['Sleepover', 'Complex Care'],
    initials: 'MK',
    color: '#0D9488',
    name: 'Michael K.',
  },
  {
    type: 'emergency',
    title: 'Emergency — Community Access',
    location: 'Perth, WA',
    time: 'Today, 2pm–6pm',
    rate: '$44/hr',
  ...
  },
  {
    ...
    time: 'Tue & Thu mornings',
    rate: '$35/hr',
    tags: ['Transport', 'Ongoing'],
    initials: 'JP',
    color: '#C2185B',
    name: 'James P.',
  },
  {
    type: 'regular',
    title: 'Therapy Support — OT',
    location: 'Canberra, ACT',
    time: 'Flexible schedule',
    rate: '$65/hr',
    tags: ['Allied Health', 'NDIS'],
    initials: 'AH',
    color: '#1D4ED8',
    name: 'Aisha H.',
  },
    ...
    <section id="marketplace" className="section-py bg-gray-soft" aria-labelledby="market-heading">
      <div className="container-xl">
        ...
        <div className="fade-up mb-5">
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1, minWidth: 180 }}>
              <i className="bi bi-search" style={{ color: 'var(--clr-muted)', fontSize: 16 }} aria-hidden="true"></i>
              <input
                type="text"
            ...
            <select
              ...
              style={{
                border: '1.5px solid var(--clr-border)', borderRadius: 10,
            ...
            <button className="btn btn-shiftify" style={{ fontSize: 14, padding: '9px 22px' }}>
              Search
            </button>
          </div>
        </div>
        ...
        <div className="row g-4">
          {listings.map((item) => (
            <div key={item.title} className="col-12 col-md-6 col-lg-4 fade-up">
              <div
              ...
              >
                ...
                <div className="d-flex align-items-center gap-3 mb-3">
                  <div className="worker-avatar" style={{ background: item.color, width:42, height:42, fontSize:14 }}>
                    {item.initials}
                  </div>
                  <div>
                    <div style={{ fontWeight: 800, fontSize: 14 }}>{item.name}</div>
                    <div style={{ fontSize: 12, color: 'var(--clr-muted)' }}>
                      <i className="bi bi-geo-alt me-1" aria-hidden="true"></i>{item.location}
                    </div>
                  </div>
                ...
                <p style={{ fontSize: 13, color: 'var(--clr-muted)', marginBottom: 14 }}>
                  <i className="bi bi-clock me-1" aria-hidden="true"></i>{item.time}
                </p>
                  ...
                  {item.tags.map(tag => (
                    <span
                      ...
                      style={{
                        ...
                        background: tag === 'Urgent' ? '#FEE2E2' : 'var(--clr-primary-xlight)',
                        color: tag === 'Urgent' ? '#B91C1C' : 'var(--clr-primary)',
                        borderRadius: 100, border: `1px solid ${tag === 'Urgent' ? '#FECACA' : 'var(--clr-primary-light)'}`,
                      }}
                    >{tag}</span>
                  ))}
                </div>
                ...
                <button
                  className={`btn w-100 ${item.type === 'emergency' ? 'btn-emergency' : 'btn-shiftify'}`}
                  style={{ fontSize: 13, padding: '10px 0' }}
                  aria-label={`Apply for ${item.title}`}
                >
                  {item.type === 'emergency' ? (
                    <><span className="dot" aria-hidden="true"></span>Respond Now</>
                  ) : 'Apply for Shift'}
                </button>
              </div>
            </div>
